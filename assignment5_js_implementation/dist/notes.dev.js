...
function () {
    ...
    this.resolve = function (data) {
      ...
      setTimeout(function () {
        ...
        var _loop = function _loop() {
          var curThenCallbackFn = _this.thenCallbackQueue.shift();

          if (_this.currentData instanceof MyPromise) {
            console.log('this is a promise');

            _this.currentData.then(function (thenData) {
              console.log('test');
              _this.currentData = curThenCallbackFn(thenData);
            });
          } else {
            console.log('test');
            _this.currentData = curThenCallbackFn(_this.currentData);
          }
        };

        while (_this.thenCallbackQueue.length > 0) {
          _loop();
        }
      }, 0);
    };
  ...
  _createClass(MyPromise, [{
    key: "then",
    value: function then(thenCallbackFn) {
...
fetch('https://jsonplaceholder.typicode.com/todos/2').then(function (response) {
  return response.json();
}).then(function (json) {
  return console.log(json);
});
fetch('https://jsonplaceholder.typicode.com/todos/3').then(function (response) {
  return response.json();
}).then(function (json) {
  return console.log(json);
});
